...
def tree_detection(img_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.error("PIL görüntü açma hatası: %s: %s", img_path, e)
        ...
        try:
            ...
            logger.error("Görüntü kaydetme hatası: %s: %s", convert_path, e)
        ...
        try:
            ...
            logger.error("Dataset yükleme hatası: %s", e)
        ...
        for path, img, im0s, vid_cap in dataset:
            try:
                ...
                try:
                    ...
                    logger.error("Çıktı kaydetme hatası: %s", e)
                    raise IOError(f"Sonuç kaydedilemedi: {e}")

            except Exception as e:
                logger.error("Ağaç algılama işlemi hatası: %s", e)
                raise
        ...
        logger.error("Tree detection genel hatası: %s", e)
