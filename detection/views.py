...
def validate_image_file(file: UploadedFile) -> bool:
    ...
    try:
        ...
        if mime not in ALLOWED_MIME_TYPES:
            logger.warning(
                "MIME type mismatch: expected image, got %s for file %s",
                mime,
                file.name,
            )
            ...
            if cached_result:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            "Veritabanı kaydetme hatası: %s", db_error)
    ...
    if request.method == "POST":
        try:
            ...
            try:
                ...
                logger.error(
                    "Çıktı dizini oluşturma hatası: %s: %s", hass[0], e)
...
def async_detection(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        logger.info("Async detection task queued: %s for %s",
                    task.id, meyve_grubu)
