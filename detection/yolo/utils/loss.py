...
class RankSort(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, delta_RS=0.50, eps=1e-10):
        ...
        for ii in order:
            ...
            # Rank of ii among pos and false positive number (bg with larger
            # scores)
            ...
            # Ranking error of example ii. target_ranking_error is always 0.
            # (Eq. 7)
            ...
            target_sorting_error = (
                torch.sum(target_sorted_order * (1 - fg_targets)) /
                rank_pos_target
            )
            ...
            if FP_num > eps:
                ...
                # For negatives, distribute error via ranking pmf (i.e.
                # bg_relations/FP_num)
            ...
            if sorting_pmf_denom > eps:
                ...
                # For positives, distribute error via sorting pmf (i.e.
                # missorted_examples/sorting_pmf_denom)
...
class aLRPLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, regression_losses, delta=1.0, eps=1e-5):
        ...
        for ii in order:
            ...
            # Compute the rank of the example within fgs and number of bgs with
            # larger scores
            rank_pos = 1 + torch.sum(fg_relations)
            FP_num = torch.sum(bg_relations)
            # Store the total since it is normalizer also for aLRP Regression
            # error
            rank[ii] = rank_pos + FP_num

            # Compute precision for this example to compute classification loss
            prec[ii] = rank_pos / rank[ii]
            # For stability, set eps to a infinitesmall value (e.g. 1e-6), then
            # compute grads
            if FP_num > eps:
                fg_grad[ii] = (
                    -(torch.sum(fg_relations * regression_losses) +
                      FP_num) / rank[ii]
                )
...
class APLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, targets, delta=1.0):
        ...
        for ii in order:
            ...
            # Compute interpolated AP and store gradients for relevant bg
            # examples
