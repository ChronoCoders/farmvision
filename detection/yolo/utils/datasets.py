...
def random_perspective(
    ...
    if n:
        ...
        if use_segments:
            ...
            xy = xy @ M.T
            xy = (xy[:, :2] / xy[:, 2:3]
                  if perspective else xy[:, :2]).reshape(n, 8)
...
def flatten_recursive(path="../coco"):
    ...
    for filepath in tqdm(glob.glob(str(Path(path)) + "/**/*.*", recursive=True)):
        shutil.copyfile(filepath, new_path / Path(filepath).name)
