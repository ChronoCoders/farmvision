...
if __name__ == "__main__":
    ...
    if opt.local_rank != -1:
        if not torch.cuda.device_count() > opt.local_rank:
            raise RuntimeError(
                f"CUDA device count {torch.cuda.device_count()} must be greater than local_rank {opt.local_rank}"
            )
    ...
    if not opt.evolve:
        ...
        for _ in range(300):
            if Path("evolve.txt").exists():
                ...
                ng = len(meta)
                v: np.ndarray[Any, np.dtype[np.float64]] = np.ones(ng)
                while all(v == 1):
                    v = (
                        (
                            g * (npr.random(ng) < mp) *
            ...
            for k, bounds in meta.items():
                hyp[k] = float(max(hyp[k], np.float64(bounds[1])))
                hyp[k] = float(min(hyp[k], np.float64(bounds[2])))
                hyp[k] = round(hyp[k], 5)
