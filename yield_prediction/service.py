...
def predict_yield(
    ...
    stress_percent = float(
        features.get("total_stressed_area_percent", 0.0) or 0.0
    )
    largest_stress_zone_ha = float(
        features.get("largest_stress_zone_ha", 0.0) or 0.0
    )
    ...
    raw_yield_kg = base_yield_kg * ndvi_factor * stress_factor * density_factor * age_factor
    ...
    tahmini_verim_kg = max(raw_yield_kg, 0.0)
    guven_skoru = confidence * 100.0

    return {
        "tahmini_verim_kg": round(tahmini_verim_kg, 2),
        "guven_skoru": round(guven_skoru, 1),
        "model_version": YIELD_MODEL_VERSION,
    }
